...
### Step 2: Generate CSR  
```bash
openssl req -new -newkey rsa:2048 -nodes -keyout retooltest.unattributed.blog.key -out retooltest.unattributed.blog.csr -subj "/C=CA/ST=Mail/L=Timbuktu/O=ACME Sprockets/OU=IT Department/CN=retooltest.unattributed.blog" -config &lt;(cat /etc/ssl/openssl.cnf &lt;(printf "[req]\ndistinguished_name=dn\n[dn]\n[ext]\nsubjectAltName=DNS:retooltest.unattributed.blog,IP:10.10.10.10\nbasicConstraints=critical,CA:FALSE\nkeyUsage=digitalSignature,keyEncipherment\nextendedKeyUsage=serverAuth")) -regexts ext
```
